...
class VpnStatusMonitor:
    ...
    #Path user auth
    c_userpass_dir_torguard = os.path.expanduser("~") + '/.config/torguard_open_vpn_userpass'
    userpass_file_name_torguard = 'torguard_openvpn_service_auth.txt'

    c_userpass_dir_vpnac = os.path.expanduser("~") + '/.config/vpnac_open_vpn_userpass'
    ...
    def get_status_vpnac(self):
        ...
        openvpn_auth_file = self.c_userpass_dir_vpnac + '/' + self.userpass_file_name_vpnac
        ...
        openvpn_auth_expr_file = self.c_userpass_dir_vpnac + '/' + self.service_expire_date_file_name
    ...
    def get_status_torguard(self):
        status = {"webuser_email": None, "webuser_password": None, "service_stored_user": None, "service_stored_password": None, "valid": None}
        openvpn_auth_file = self.c_userpass_dir_torguard + '/' + self.userpass_file_name_torguard
        if os.path.isfile(openvpn_auth_file):
            file = open(openvpn_auth_file, "r")
            lines = file.readlines()
            status["service_stored_user"] = lines[0].replace('\n', '').replace('\r', '')
        ...
        openvpn_auth_expr_file = self.c_userpass_dir_torguard + '/' + self.service_expire_date_file_name
        if os.path.isfile(openvpn_auth_expr_file):
            file = open(openvpn_auth_expr_file, "r")
            lines = file.readlines()
            status['valid'] = lines[0].replace('\n', '').replace('\r', '')
    ...
    def get_status_purchased(self):
        ...
        if status_torguard['web_user_email'] != None: purchased.append('torguard') 
        status_vpnac = self.get_status_vpnac()
        if status_vpnac['web_user_email'] != None: purchased.append('vpnac')
        return purchased
