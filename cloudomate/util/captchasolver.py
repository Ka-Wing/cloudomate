...
class CaptchaSolver(object):
    ...
    def get_balance(self):
        ...
        response = requests.post(
            "https://api.anti-captcha.com/get_balance",
            json={"clientKey": self._client_key})

        # Checks response of HTTP request
    ...
    def solve_captcha_text_case_sensitive(self, full_image_file_path):
        # Encodes captcha image before sending it
        ...
        # Creates new Captcha solving task using the API
    ...
    def _create_task_captcha_text_case_sensitive(
            self, base64_image_string):
        ...
        if response.status_code == requests.codes.ok:
            response_json = json.loads(response.text)
            if response_json["errorId"] == 0:
                print("Successful, captcha task was created:"
                      + response.text)
                return response_json["taskId"]
            elif response_json["errorCode"] == \
...
class ReCaptchaSolver(CaptchaSolver):
    ...
    def _create_task_google_recaptcha(self, website_url, website_key):
        ...
        if response.status_code == requests.codes.ok:
            ...
            if response_json["errorId"] == 0:
                ...
                time.sleep(15)
                return self._create_task_google_recaptcha(
    ...
    def solve_google_recaptcha(self, website_url, website_key):
        ...
        task_id = \
            self._create_task_google_recaptcha(website_url, website_key)
